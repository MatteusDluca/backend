...
export class ClientsService {
  ...
  async create(createClientDto: CreateClientDto) {
    // Verificar se já existe cliente com o email, CPF/CNPJ ou telefone
    ...
    if (existingClient) {
      throw new ConflictException('E-mail, CPF/CNPJ ou telefone já cadastrado')
    }
    ...
    const state = await this.prisma.state.upsert({
      ...
      create: {
        ...
        uf: createClientDto.state.substring(0, 2).toUpperCase(), // Simplificação
      },
    })
  ...
  async update(id: string, updateClientDto: UpdateClientDto) {
    ...
    // Verificar conflitos de dados únicos
    if (
      ...
      if (conflict) {
        throw new ConflictException(
          'E-mail, CPF/CNPJ ou telefone já cadastrado por outro cliente',
        )
      }
    }
    ...
    if (
      ...
      // Se tem endereço, atualiza; se não, cria um novo
      if (existingClient.address) {
        // Verificar e atualizar rua se necessário
        ...
        // Verificar e atualizar cidade/estado se necessário
        let cityId = existingClient.address.cityId
        if (updateClientDto.city || updateClientDto.state) {
          // Obter cidade atual para obter estado relacionado
          ...
          if (!currentCity) {
            throw new NotFoundException(
              `Cidade com ID ${existingClient.address.cityId} não encontrada`,
            )
          }
        ...
        await this.prisma.address.update({
          ...
          data: {
            ...
            complement:
              updateClientDto.complement !== undefined
                ? updateClientDto.complement
        ...
        if (
          ...
          throw new BadRequestException(
            'Para criar um novo endereço, todos os campos são obrigatórios (street, number, zipCode, city, state)',
          )
        }
        ...
        // Atualizar cliente com o novo endereço
    ...
    // Atualizar medidas se necessário
    if (existingClient.measurements) {
      const measurementsUpdate: Record<string, any> = {}

      // Adicionar apenas os campos que precisam ser atualizados
      ...
      if (Object.keys(measurementsUpdate).length > 0) {
        await this.prisma.measurements.update({
          where: { id: existingClient.measurementsId as string },
          data: measurementsUpdate,
        })
      }
    } else if (
      updateClientDto.shoulder !== undefined ||
      ...
      // Atualizar cliente com as novas medidas
      await this.prisma.client.update({
        where: { id },
